...
AppTitle "SCP - Containment Breach version "+VersionNumber

If CustomIntro Then
	ClsColor CustomIntroClsColorR, CustomIntroClsColorG, CustomIntroClsColorB
	Cls
	img_ = LoadImage_Strict(CustomIntroImagePath)
...
Function ExecConsole(cin$, silent% = False)
	If Not silent Then
		CreateConsoleMsg(cin,255,255,0,True)
	End If

	If Instr(cin, " ") > 0 Then
		StrTemp$ = Lower(Left(cin, Instr(cin, " ") - 1))
	Else
		StrTemp$ = Lower(cin)
	End If
	...
	Select Lower(StrTemp)
		...
		Case "hidedistance"
			;[Block]
			HideDistance = Float(Right(cin, Len(cin) - Instr(cin, " ")))
...
;					EndIf
		...
		Case "chelp":
			...
			CreateConsoleMsg("cnpc.remove - deletes controllable NPC.")
			CreateConsoleMsg("cnpc.teleport - teleports controllable NPC to player.")
			CreateConsoleMsg("cnpc.follow [on/true/1 off/false/0] - toggles (or sets) player follow a controllable NPC.")
		...
		Case "secondarylight"
			...
			Select StrTemp
				...
				Default
					SecondaryLightOn = Not SecondaryLightOn
			End Select
		...
		Case "props.create"
			...
			If StrTemp2 = StrTemp Or StrTemp2 = "" Then StrTemp2 = 1

			Select StrTemp
				Case "keyboard"
					prop = LoadMesh("GFX\map\Props\keyboard.x")
					EntityTexture prop, LoadTexture("GFX\map\Props\keyboard.jpg")

				Case "cabinet.a"
					prop = LoadMesh("GFX\map\Props\cabinet_a.x")
					EntityTexture prop, LoadTexture("GFX\map\Props\cabinet_a.jpg")

				Case "cabinet.b"
					prop = LoadMesh("GFX\map\Props\cabinet_b.x")
					EntityTexture prop, LoadTexture("GFX\map\Props\cabinet_a.jpg")

				Default
					;FreeEntity prop

					CreateConsoleMsg("Unknown prop type.", 255, 0, 0)

			End Select

			If prop <> 0 Then
				EntityType prop, HIT_MAP
				PositionEntity prop, EntityX(Collider), EntityY(Collider), EntityZ(Collider)
				
				t_ent_scale = Float(StrTemp2) / MeshWidth(prop)
				ScaleEntity prop, t_ent_scale, t_ent_scale, t_ent_scale
			...
			If ctrl_npc = Null Then
				Select Lower(Right(cin, Len(cin) - Instr(cin, " "))) 
					...
					Case "gonzales"
						...
						ChangeNPCTextureID(ctrl_npc, 0)
						CreateConsoleMsg("Gonzales (controllable) spawned.", 0, 255, 0)

					Case "corpse"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 1)
						CreateConsoleMsg("Corpse (controllable) spawned.", 0, 255, 0)

					Case "scientist.1"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 2)
						CreateConsoleMsg("Scientist type 1 (controllable) spawned.", 0, 255, 0)

					Case "scientist.2"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 3)
						CreateConsoleMsg("Scientist type 2 (controllable) spawned.", 0, 255, 0)

					Case "janitor"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 4)
						CreateConsoleMsg("Janitor (controllable) spawned.", 0, 255, 0)

					Case "106victim"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 5)
						CreateConsoleMsg("SCP-106 Victum (controllable) spawned.", 0, 255, 0)

					Case "classd.2"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 6)
						CreateConsoleMsg("D-Class type 2 (controllable) spawned.", 0, 255, 0)

					Case "035victim"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						ChangeNPCTextureID(ctrl_npc, 7)
						CreateConsoleMsg("SCP-035 Victim (controllable) spawned.", 0, 255, 0)

					Case "classd.1"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						CreateConsoleMsg("D-Class type 1 (controllable) spawned.", 0, 255, 0)

					Case "classd.3"
						ctrl_npc = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						tex = LoadTexture_Strict("GFX\npcs\classd3.jpg")
						EntityTexture ctrl_npc\obj, tex
						FreeTexture tex
						CreateConsoleMsg("D-Class type 3 (controllable) spawned.", 0, 255, 0)
						
					Case "clerk"
						ctrl_npc = CreateNPC(NPCtypeClerk, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
						CreateConsoleMsg("Clerk (controllable) spawned.", 0, 255, 0)
		...
		Case "cnpc.remove"
			If ctrl_npc <> Null Then
				...
				CreateConsoleMsg("Current controllable NPC removed.", 0, 255, 0)
			Else
		...
		Case "cnpc.teleport"
			If ctrl_npc <> Null Then
				PositionEntity ctrl_npc\Collider, EntityX(Collider), EntityY(Collider), EntityZ(Collider)
				ResetEntity ctrl_npc\Collider

				CreateConsoleMsg("Current controllable NPC teleported to player.", 0, 255, 0)
			Else
				CreateConsoleMsg("Current controllable NPC doesn`t exist.", 255, 0, 0)
			End If
		...
		Case "cnpc.follow"
			If ctrl_npc <> Null Then
				...
				Select StrTemp
					Case "on", "1", "true"
						ctrl_npc_follow = True						
					Case "off", "0", "false"
						ctrl_npc_follow = False	
					Default
						ctrl_npc_follow = Not ctrl_npc_follow
				End Select

				If ctrl_npc_follow Then
					;EntityParent Collider, ctrl_npc\Collider, False
					;PositionEntity Collider, EntityX(ctrl_npc\Collider), EntityY(ctrl_npc\Collider) + 0.5, EntityZ(ctrl_npc\Collider) - 0.5

					ctrl_npc_follow_entity = CreateCube(ctrl_npc\Collider)
					MoveEntity ctrl_npc_follow_entity, 0, 0.5, -0.5
					ScaleEntity ctrl_npc_follow_entity, 0.1, 0.1, 0.1
					EntityColor ctrl_npc_follow_entity, 255, 0, 255
					HideEntity ctrl_npc_follow_entity
					
					CreateConsoleMsg("Player following current controllable NPC.", 0, 255, 0)
				Else
					;EntityParent Collider, 0, True
					FreeEntity ctrl_npc_follow_entity
					ctrl_npc_follow_entity = 0

					CreateConsoleMsg("Player no following current controllable NPC.", 0, 255, 0)
				End If
			Else
				CreateConsoleMsg("Current controllable NPC doesn`t exist.", 255, 0, 0)
			End If
		...
		Case "cnpc.follow.playerrotating"
			...
			Select StrTemp
				Case "on", "1", "true"
					ctrl_npc_follow_enable_player_rotating = True						
				Case "off", "0", "false"
					ctrl_npc_follow_enable_player_rotating = False	
				Default
					ctrl_npc_follow_enable_player_rotating = Not ctrl_npc_follow_enable_player_rotating
			End Select

			If ctrl_npc_follow_enable_player_rotating Then
				CreateConsoleMsg("Rotating player by follow point enabled.", 0, 255, 0)
			Else
				CreateConsoleMsg("Rotating player by follow point disabled.", 0, 255, 0)
			End If

		Case "cnpc.follow.followpoint.show"
			If ctrl_npc_follow Then
				ShowEntity ctrl_npc_follow_entity
				CreateConsoleMsg("Follow point entity showed.", 0, 255, 0)
			Else
				CreateConsoleMsg("Follow mode not enabled.", 255, 0, 0)
			End If

		Case "cnpc.follow.followpoint.hide"
			If ctrl_npc_follow Then
				HideEntity ctrl_npc_follow_entity
				CreateConsoleMsg("Follow point entity hid.", 0, 255, 0)
			Else
				CreateConsoleMsg("Follow mode not enabled.", 255, 0, 0)
			End If
		...
		Case "cnpc.follow.followpoint.position.move"
			...
			If ctrl_npc_follow Then
				MoveEntity ctrl_npc_follow_entity, Float(StrTemp), Float(StrTemp2), Float(StrTemp3)
				CreateConsoleMsg("Follow point entity moved on (X|Y|Z) " + Float(StrTemp) + " " + Float(StrTemp2) + " " + Float(StrTemp3) + ".", 0, 255, 0)
			Else
				CreateConsoleMsg("Follow mode not enabled.", 255, 0, 0)
			End If
		...
		Case "cnpc.follow.followpoint.position.set"
			...
			If ctrl_npc_follow Then
				PositionEntity ctrl_npc_follow_entity, Float(StrTemp), Float(StrTemp2), Float(StrTemp3)
				CreateConsoleMsg("Follow point entity position set on (X|Y|Z) " + Float(StrTemp) + " " + Float(StrTemp2) + " " + Float(StrTemp3) + ".", 0, 255, 0)
			Else
				CreateConsoleMsg("Follow mode not enabled.", 255, 0, 0)
			End If
		...
		Case "cnpc.follow.followpoint.reset"
			If ctrl_npc_follow Then
				...
				;RotateEntity ctrl_npc_follow_entity, 0, 0, 0
				CreateConsoleMsg("Follow point position resetted.", 0, 255, 0)
			Else
				CreateConsoleMsg("Follow mode not enabled.", 255, 0, 0)
			End If
		...
		Case "sc.create"
			...
			ResetEntity sc\ScrObj
			Select StrTemp
				Case "allow", "1", "true", "yes"
					sc\AllowSaving = True				
				Case "deny", "0", "false", "no"
					sc\AllowSaving = False
				Default
					sc\AllowSaving = False
			End Select
			
			CreateConsoleMsg("Created new camera with monitor (allow saving: " + sc\AllowSaving + ").", 0, 255, 0)

		; Security Cameras

		Case "sc.camera.nearest.remove"
			sc = GetNearestSCToEntity(Collider)
			
			FreeEntity sc\obj
			FreeEntity sc\MonitorObj
			FreeEntity sc\BaseObj
			FreeEntity sc\CameraObj
			If sc\Screen Then
				FreeEntity sc\ScrObj
			End If
			Delete sc
		...
		Case "sc.camera.nearest.rotation.set"
			...
			RotateEntity sc\CameraObj, Float(StrTemp), EntityYaw(sc\CameraObj), EntityPitch(sc\CameraObj)
		...
		Case "sc.camera.nearest.rotation.turn"
			...
			TurnEntity sc\CameraObj, Float(StrTemp), 0, 0
		...
		Case "sc.camera.nearest.turnangle.set"
			...
			;sc\angle = Float(StrTemp)

			CreateConsoleMsg("Nearest camera turn set on angle " + Float(StrTemp) + ".", 0, 255, 0)
		
		; Security Monitors

		Case "sc.monitor.nearest.remove"
		...
		Case "scp079.screen.image.set.fromdefault"
			Select Lower(Right(cin, Len(cin) - Instr(cin, " ")))
				...
				Case "cross", "x", "1"
					texScr079 = OldAiPics(1)

				Default
					CreateConsoleMsg("Incorrect image.", 255, 0, 0)
					Return
		...
		Case "scp079.screen.image.clear"
			...
			For rm.Rooms = Each Rooms
				If rm\RoomTemplate\Name = "room079"
					HideEntity rm\Objects[1]

					CreateConsoleMsg("Cleared screen texture of SCP-079 instance number " + Str(ctr) + ".", 0, 255, 0)
...
Function ExecConsoleFile(filepath$)
	...
	Repeat
		...
		command$ = Left(line, size)
		If size = 0 Then command = line
...
If EnableMusic Then MusicCHN = StreamSound_Strict("SFX\Music\"+Music(2)+".ogg",MusicVolume,Mode)
		...
		If d\dist < HideDistance*2 Or d\IsElevatorDoor>0 Then ;Make elevator doors update everytime because if not, this can cause a bug where the elevators suddenly won't work, most noticeable in room2tunnel - ENDSHN
			...
			If d\open Then
				...
				Else
					...
					EndIf
					If SCP079CanCloseDoors and d\AutoClose And RemoteDoorOn = True Then
...
Repeat
	...
	If MainMenuOpen Then
		If ShouldPlay = 21 Then
			...
			If EnableSFX EndBreathCHN = PlaySound(EndBreathSFX)
			ShouldPlay = 66
		ElseIf ShouldPlay = 66
			If (Not ChannelPlaying(EndBreathCHN)) Then
				If EnableSFX FreeSound(EndBreathSFX)
				ShouldPlay = 11
			EndIf
		Else
			ShouldPlay = 11
	...
	Else
		...
		If PlayerRoom\RoomTemplate\Name <> "pocketdimension" And PlayerRoom\RoomTemplate\Name <> "gatea" And PlayerRoom\RoomTemplate\Name <> "exit1" And (Not MenuOpen) And (Not ConsoleOpen) And (Not InvOpen) Then 
			...
			If Rand(1500) = 1 and EnableAmbientSFX Then
				For i = 0 To 5
			...
			If Rand(50000) = 3 and FacilityLightBlinkingBySCP079 Then
		...
		If (Not MenuOpen) And (Not InvOpen) And (OtherOpen=Null) And (SelectedDoor = Null) And (ConsoleOpen = False) And (Using294 = False) And (SelectedScreen = Null) And EndingTimer=>0 Then
			...
			If PlayerRoom\RoomTemplate\Name<>"pocketdimension" Then
				CameraClsColor(Camera, ClsColorR, ClsColorG, ClsColorB)
			EndIf
	...
	If KeyHit(KEY_CONSOLE) Then
			If CanOpenConsole Or BypassOpeningConsole
				If ConsoleOpen Then
					UsedConsole = True
					ResumeSounds()
					MouseXSpeed() : MouseYSpeed() : MouseZSpeed() : mouse_x_speed_1#=0.0 : mouse_y_speed_1#=0.0
	...
	If BorderlessWindowed Then
		If (RealGraphicWidth<>GraphicWidth) Or (RealGraphicHeight<>GraphicHeight) Then
			...
			ClsColor ClsColorR,ClsColorG,ClsColorB : Cls
	...
	ElseIf ScreenGamma<1.0 Then ;todo: maybe optimize this if it's too slow, alternatively give players the option to disable gamma
		...
		ScaleRender(-1.0/Float(RealGraphicWidth),1.0/Float(RealGraphicWidth),2048.0 / Float(RealGraphicWidth),2048.0 / Float(RealGraphicWidth))
		SetBuffer(TextureBuffer(fresize_texture2))
		ClsColor ClsColorR,ClsColorG,ClsColorB
		Cls
		SetBuffer(BackBuffer())
	EndIf
		...
		Case "room2closets"
			...
			If e\EventState = 0
				...
				ElseIf e\EventStr = "load1"
					QuickLoadPercent = 20
					If EnableAmbientSFX e\room\NPC[0]\Sound=LoadSound_Strict("SFX\Room\Storeroom\Escape1.ogg")
					e\EventStr = "load2"
				ElseIf e\EventStr = "load2"
					QuickLoadPercent = 35
				...
				ElseIf e\EventStr = "load4"
					QuickLoadPercent = 80
					If EnableAmbientSFX e\room\NPC[1]\Sound=LoadSound_Strict("SFX\Room\Storeroom\Escape2.ogg")
					e\EventStr = "load5"
...
Function DrawEnding()
	...
	If EndingTimer<-200 Then
		...
		If EndingScreen = 0 Then
			...
			CurrMusicVolume = MusicVolume
			If EnableMusic Then StopStream_Strict(MusicCHN)
			If EnableMusic Then MusicCHN = StreamSound_Strict("SFX\Music\"+Music(23)+".ogg",CurrMusicVolume,0)
...
Function LoadEntities()
	...
	;
	;PlayerModel = CopyEntity(ClassDObj)
	;;EntityTexture PlayerModel, LoadTexture_Strict("GFX\npcs\classd1.jpg")
	;EntityParent PlayerModel, Collider, false
	;
	;Local scale_playermodel# = 0.5 / MeshWidth(PlayerModel)
	;ScaleEntity PlayerModel, scale_playermodel, scale_playermodel, scale_playermodel
	;MeshCullBox (PlayerModel, -MeshWidth(ClassDObj), -MeshHeight(ClassDObj), -MeshDepth(ClassDObj), MeshWidth(ClassDObj)*2, MeshHeight(ClassDObj)*2, MeshDepth(ClassDObj)*2)

	;PositionEntity PlayerModel, scale_playermodel / 2, scale_playermodel / 2, scale_playermodel / 2
	;RotateEntity PlayerModel, 0, 180, 0

	;ShowEntity PlayerModel
...
Function UpdateMTF%()
	If Not UpdateMTFEnabled Return
...
Function CreateDecal.Decals(id%, x#, y#, z#, pitch#, yaw#, roll#)
	;If EnableDecals Then Return Null
	...
	If (Not EnableDecals) Or (Not EnabledDecalTextures(id)) Then HideEntity d\obj ; (id >= 4 And id <= 6)
...
Function CatchErrors(location$)
	...
	If Len(errStr)>0 Then
		...
		Local errormsg$ = "Blitz3D Error! Details in "+Chr(34)+ErrorFile+Chr(34)
		If SendErrorsToConsole Then
			CreateConsoleMsg(errormsg, 255, 0, 0)
		Else
			Msg = errormsg
			MsgTimer = 20*70
		End If
