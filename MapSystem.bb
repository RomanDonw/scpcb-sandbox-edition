...
Dim MapGridValues%(MapWidth+1, MapHeight+1)
...
Function CreateRoom.Rooms(zone%, roomshape%, x#, y#, z#, name$ = "", enable_filling% = True)
	CatchErrors("Uncaught (CreateRoom)")

	;If Not GetINIInt(OptionFile, "rooms", name) Return
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "room1archive"
			...
			r\RoomDoors[0] = CreateDoor(r\zone,r\x,r\y,r\z - 528.0 * RoomScale,0,r,False,False,1)
		...
		If x<16 And z < 16 Then
			For i = 0 To MaxRoomEmitters-1
				If r\SoundEmitter[i]<>0 Then 
					...
					If dist < r\SoundEmitterRange[i] and EnableRoomAmbientSFX Then
...
Function AddLight%(room.Rooms, x#, y#, z#, ltype%, range#, r%, g%, b%, ltextype% = 0)
	Local i
...
Type SecurityCams
	...
	Field EnableSCP079PicOnScreen% = True
End Type
...
Function CreateSecurityCam.SecurityCams(x#, y#, z#, r.Rooms, screen% = False, EnableSCP079PicOnScreen% = True)
	...
	sc\Screen = screen
	sc\EnableSCP079PicOnScreen = EnableSCP079PicOnScreen
	If screen Then
		sc\AllowSaving = True
	...
	For sc.SecurityCams = Each SecurityCams
		...
		Else
			...
			If close = True Then
				If sc\Screen Then
					...
					If (BlinkTimer > - 5 And EntityInView(sc\ScrObj, Camera)) Then
						If EntityVisible(Camera,sc\ScrObj) Then
							If (sc\CoffinEffect=1 Or sc\CoffinEffect=3) And (Not Wearing714) And (WearingHazmat<3) And (WearingGasMask<3) And (Not DisableCoffinEffect) Then
								If BlinkTimer > - 5
					...
					If sc\InSight And sc\CoffinEffect=0 Or sc\CoffinEffect=2 Then
						...
						If Rand(500) = 1 Then
							If EnableSCP079PicOnScreens and sc\EnableSCP079PicOnScreen Then EntityTexture(sc\ScrOverlay, OldAiPics(0))
						End If
						...
						Else
							If sc\soundCHN = 0 Then
								If EnableSecurityCamerasHorrorSFX sc\soundCHN = PlaySound_Strict(LoadTempSound("SFX\SCP\079\Broadcast"+Rand(1,3)+".ogg"))
				...
				If (Not sc\InSight) and EnableSecurityCamerasBasicSFX Then sc\soundCHN = LoopSound2(CameraSFX, sc\soundCHN, Camera, sc\CameraObj, 4.0)
			Else
...
Function CreateMap()
	...
	For y = 0 To MapHeight
		For x = 0 To MapWidth
			MapGridValues(x, y) = MapTemp(x, y)
		Next
	Next

	If ShowFacilityMapOnLoad Then 
		Repeat
			Cls
